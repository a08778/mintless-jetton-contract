...
- `BURN_NOTIFICATION_BITS` Bits occupied by the [burn_notification](https://github.com/ton-blockchain/mintless-jetton-contract/blob/77763e6b2df6cf96b0840c7306844751200ff046/contracts/jetton.tlb#L76-L81) message
- `BURN_NOTIFICATION_CELLS` Cells occupied by the [burn_notification](https://github.com/ton-blockchain/mintless-jetton-contract/blob/77763e6b2df6cf96b0840c7306844751200ff046/contracts/jetton.tlb#L76-L81) message
...
> ⚠️ **Important notice:**
> Mintless jetton uses shard brute-force mechanism for a wallet to end up in the same shard as it's owner,
> In order to estimate gas consumption of this process, special git branch
> is used, to estimate maximum address calculations  costs.
> So, all measurements are supposed to be executed on the `same_shard_measure`
> and resulting values should be propagated on the actual release branch.
> Note that Claim.ts tests will fail on `same_shard_measure` due to modifications mentioned above
...
1. Run [StateInit](https://github.com/Trinketer22/mintless-jetton/blob/same_shard_measure/sandbox_tests/StateInit.spec.ts) test set.
...
- Adjust storage constant is main test suite [walletStats](https://github.com/Trinketer22/mintless-jetton/blob/b60008a18a2a4825ae5719abca8a4153f2e6959a/sandbox_tests/JettonWallet.spec.ts#L80) and [stateInitStats](https://github.com/Trinketer22/mintless-jetton/blob/b60008a18a2a4825ae5719abca8a4153f2e6959a/sandbox_tests/JettonWallet.spec.ts#L85) accordingly

### Other code changes

Run the [wallet](https://github.com/Trinketer22/mintless-jetton/blob/same_shard_measure/sandbox_tests/JettonWallet.spec.ts) test suite
...
- Adjust send transfer gas [fee](https://github.com/Trinketer22/mintless-jetton/blob/b60008a18a2a4825ae5719abca8a4153f2e6959a/sandbox_tests/JettonWallet.spec.ts#L766) in tests
- Adjust transfer receive transfer gas [fee](https://github.com/Trinketer22/mintless-jetton/blob/b60008a18a2a4825ae5719abca8a4153f2e6959a/sandbox_tests/JettonWallet.spec.ts#L777) in tests.
- Adjust burn and burn notification [fees](https://github.com/Trinketer22/mintless-jetton/blob/b60008a18a2a4825ae5719abca8a4153f2e6959a/sandbox_tests/JettonWallet.spec.ts#L1057-L1058) in tests if necessary
- Re-run the test and expect no test failures

### Using current code without library

As mentioned in main [README](https://github.com/ton-blockchain/mintless-jetton-contract/tree/main), jetton wallet utilizes [library cell](https://docs.ton.org/v3/documentation/data-formats/tlb/library-cells#introduction)
...
+const SEND_TRANSFER_GAS_CONSUMPTION    = 30672;
...
@@ -103,7 +103,7 @@ describe('JettonWallet', () => {
...
@@ -763,7 +763,7 @@ describe('JettonWallet', () => {
             success: true
         });
...
@@ -774,7 +774,7 @@ describe('JettonWallet', () => {
             success: true
         });
...
@@ -1054,7 +1054,7 @@ describe('JettonWallet', () => {
